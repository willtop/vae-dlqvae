...
def construct_vae_encoder(conv_channels, latent_dim):
    in_dim = 3 
    encoder_conv_list = []
    for h_dim in conv_channels:
    ...
    encoder_fc_mu = nn.Linear(conv_channels[-1] * conv_out_size**2, latent_dim)
    encoder_fc_var = nn.Linear(conv_channels[-1] * conv_out_size**2, latent_dim)
